...
If you like this (otherwise too haha), you can check out more of my stuff at [marvosyntactical.github.io](https://marvosyntactical.github.io). If you wanna chat about this or even want to buy me a coffee so I can deploy a better model on this page, just send me an email.
                        ...
                        dcc.Store(id="focus-token", data=None),       # override: show up to this token
                    ]),
...
def build_fig_from_step(step, tokens_shown: int) -> go.Figure:
    """
    Rebuild a clean 3D figure from step["X"] cropping to the first `tokens_shown` tokens.
    Preserves the LN slider via frames and adds a Play updatemenu.
    """
...
def on_prev_next(prev_clicks, next_clicks, steps, idx):
    ...
    # clear any manual focus when stepping
    return idx, None
...
def on_click_token(nclicks, steps):
    ...
    # earliest step where len(tokens) >= tok_i+1
    new_idx = len(steps) - 1
...
def on_step_change(steps, idx, focus_tok, theme_value):
    ...
    # Determine how many tokens to show in the 3D plot
    # - if user focused a token: show up to that token (inclusive)
    # - else: show whatever this step produced (tokens so far)
    ...
    # Token strip should show ALL tokens (from the last step), while highlighting:
    # - the focused token if any; otherwise the newest token for this step.
    full = steps[-1]
    ...
    # Theme is already dark in build_fig_from_step; keep as-is for dark mode.
