...
app.layout = html.Div(
    ...
    children=[
        dcc.Markdown(
            ...
            /* Inputs — keep dark theme readable */
            ...
            html.Div(
                ...
                children=[
                    ...
                    html.Div(children=[
                        ...
                        # infobox just under buttons (KEEPING YOUR TEXT)
...
To get started, press ⏯ and give it a minute or two to update the figure. [Spoiler](/assets/argos.gif) of what it looks like.

**What you're seeing**
Each dot is the hidden vector currently associated with a given input **token** (prompt + generated so far).

We capture activations after every **LayerNorm** (usually after each **Attention** and each **MLP**), stack them, run **PCA** to 3D, and project to the **unit sphere**.
...
# Clickable token strip → jump to the step where that token first exists
@app.callback(
    Output("step-index", "data", allow_duplicate=True),
...
def on_click_token(n_clicks_list, steps):
    if not steps or not n_clicks_list:
        return no_update
    # Which token was clicked?
    trigger = ctx.triggered_id
    if not isinstance(trigger, dict) or "index" not in trigger:
        return no_update
    tok_i = int(trigger["index"])
    # Find earliest step where token i exists (len(tokens) >= i+1)
...
def on_step_change(steps, idx, theme_value):
    ...
    for i, (tok, col) in enumerate(zip(safe_tokens, step["colors"])):
        ...
        chips.append(html.Span(
            ...
            n_clicks=0  # required for ALL-pattern inputs
        ))
