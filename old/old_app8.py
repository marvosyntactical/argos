...
# Amber controls panel (legible black-on-amber)
AMBER_BG = "#F0B45A"      # tasteful amber
AMBER_TEXT = "#111"       # near-black text
...
app.layout = html.Div(
    ...
    children=[
        ...
        dcc.Markdown(
            ...
            /* BIG brand; also force inline on element as backup */
            .brand {{
            ...
            /* Switch look for one-off dcc.Checklist toggles inside #controls */
        ...
        html.Div(style={"padding": "1.2rem"}, children=[
            # force BIG inline font in addition to CSS class, so frameworks can’t shrink it
...
More: [marvosyntactical.github.io](https://marvosyntactical.github.io)
                    ...
                    # RIGHT — GIF until first results, then graph+text
...
# Immediate "Generating..." indicator (non-blocking)
@app.callback(
    Output("busy","data"),
...
# Swap GIF -> Graph when we have data
@app.callback(
    Output("gif-wrap","style"),
    Output("sphere","style"),
    Input("has-data","data")
)
